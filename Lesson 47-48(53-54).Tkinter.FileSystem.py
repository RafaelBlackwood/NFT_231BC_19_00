...
#Practise:
'''
1. Сделать так,чтобы при переходи в разные меню информация в энтри полях стиралась
2. Сделать так, чтобы если пользователь не ввел какой-то из данных выдавалась ошибка
3. Сделать так, чтобы была проверка на эмейл и корректность его написания
4. Создать логин окно
'''